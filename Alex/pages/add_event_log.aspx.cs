...
{
    public partial class add_event_log : System.Web.UI.Page
    {
        ...
        protected void BtnSaveTimeLine_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                SqlCommand cmd = new SqlCommand("sp_ms_event_log_add", con);
                cmd.CommandType = CommandType.StoredProcedure;
                con.Open();

                cmd.Parameters.AddWithValue("@event_log_date", tbTimelineDate.Text.ToString());
                ...
                cmd.ExecuteNonQuery();
                TimeLineBindData();
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Message", "alert('Event Log Saved Successfully');", true);
            }
            catch (Exception ex)
            {
        ...
        protected void TimeLineBindData()
        {
            ...
            try
            {
                SqlCommand cmd = new SqlCommand("sp_ms_event_log_event_active_year_term", con);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        protected void GridViewTimeLine_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["conStr"].ConnectionString);
            con.Open();
            SqlCommand cmd = new SqlCommand("sp_ms_event_log_edit", con);
            ...
            cmd.Parameters.Add(new SqlParameter() { ParameterName = "event_log_id", Value = GridViewTimeLine.Rows[e.RowIndex].Cells[0].Text });
            ...
            cmd.Parameters.Add(new SqlParameter() { ParameterName = "event_log_date", Value = tbDATE.Text });
        ...
        protected void GridViewTimeLine_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["conStr"].ConnectionString);
            con.Open();
            SqlCommand cmd = new SqlCommand("sp_ms_event_log_delete", con);
            GridViewRow row = GridViewTimeLine.Rows[e.RowIndex] as GridViewRow;
            cmd.Parameters.Add(new SqlParameter() { ParameterName = "event_log_id", Value = GridViewTimeLine.Rows[e.RowIndex].Cells[0].Text });
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.ExecuteNonQuery();
