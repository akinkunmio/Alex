...
{
    public partial class employee_pay_info : System.Web.UI.Page
    {
        int lvl = 0;
        ...
        private void Employee_payrollList()
        {
            ...
            try
            {
                SqlCommand cmd = new SqlCommand("ms_hr_payroll_monthly_view_year_month", con);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        protected void btnRunPayroll_Click(object sender, EventArgs e)
        {
          ...
          SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["conStr"].ConnectionString);
              try
              {
                  SqlCommand cmd = new SqlCommand("sp_ms_payroll_monthly_run", con);
                  cmd.CommandType = CommandType.StoredProcedure;
                  using (SqlDataAdapter sda = new SqlDataAdapter())
                  {
                      con.Open();
       ...
       private void PayrollDelete()
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("sp_ms_payroll_last_run_delete", con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@year", ddlYear.SelectedItem.ToString());
                cmd.Parameters.AddWithValue("@month", ddlMonth.SelectedItem.ToString());
                cmd.ExecuteNonQuery();
        ...
        protected void btnPaySalaries_Click(object sender, EventArgs e)
        {
            ...
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("sp_ms_hr_payment_add_all", con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@year", ddlYear.SelectedItem.ToString());
                cmd.Parameters.AddWithValue("@month", ddlMonth.SelectedItem.ToString());
                cmd.ExecuteNonQuery();
