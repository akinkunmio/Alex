...
{
    public partial class reportcard_nursery : System.Web.UI.Page
    {
        ...
        private MyStuDataSet GetNurseryRatings()
        {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["constr"].ConnectionString))
                try
                {
                    using (SqlCommand cmd = new SqlCommand("sp_ms_person_report_card_nursery_assessment", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@reg_id", SqlDbType.Int).Value = RegID;
                        using (SqlDataAdapter sda = new SqlDataAdapter())
                        {
                            ...
                            cmd.Connection = con;
                            sda.SelectCommand = cmd;
                            dt.sp_ms_person_report_card_nursery_assessment.Clear();
                            dt.EnforceConstraints = false;
                            sda.Fill(dt.Tables["sp_ms_person_report_card_nursery_assessment"]);
                        }
                    }
        ...
        private void Info()
        {
            ...
            RptViewerRptCard.LocalReport.ReportPath = Server.MapPath("../Report/ReportCard_Nursery.rdlc");
            ...
            ReportDataSource datasource6 = new ReportDataSource("RatingsDataSet", dsNurseryRatings.Tables[16]);
