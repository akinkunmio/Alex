...
{
    public partial class report_cards_no_pos_cumulative : System.Web.UI.Page
    {
        ...
        private MyStuDataSet GetData()
        {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["constr"].ConnectionString))
                try
                {
                    using (SqlCommand cmd = new SqlCommand("sp_ms_person_assessment_breakdown_cum_v7", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        ...
                        using (SqlDataAdapter sda = new SqlDataAdapter())
                        {
                            ...
                            dt.sp_ms_person_assessment_breakdown_cum_v7.Clear();
                            dt.EnforceConstraints = false;
                            sda.Fill(dt.Tables["sp_ms_person_assessment_breakdown_cum_v7"]);
                        }
                    }
        ...
        private MyStuDataSet GetAssessment1Data()
        {
        ...
        private MyStuDataSet GetCumTotalsData()
        {
            MyStuDataSet dt = new MyStuDataSet();
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["constr"].ConnectionString))
                try
                {
                    using (SqlCommand cmd = new SqlCommand("sp_ms_person_assessment_summary_cum_v7", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@ay_term_id", ddlTerm.SelectedValue);
                        cmd.Parameters.AddWithValue("@class", ddlFormClass.SelectedItem.Text);
                        using (SqlDataAdapter sda = new SqlDataAdapter())
                        {
                            ...
                            dt.EnforceConstraints = false;
                            sda.Fill(dt.Tables["sp_ms_person_assessment_summary_cum_v7"]);
                        }
                    }
        ...
        private void Info()
        {
            ...
            RptViewerRptCard.LocalReport.ReportPath = Server.MapPath("~/Report/ReportCards_Students_No_PosCumulative.rdlc");
            MyStuDataSet dsAssessmentcum = GetData();
            MyStuDataSet dsAssessment1 = GetAssessment1Data();
            MyStuDataSet dsAssessment2 = GetAssessement2Data();
            MyStuDataSet dsAttendance = GetAttendanceData();
            MyStuDataSet dsCumTotals = GetCumTotalsData();
            MyStuDataSet dsTotals = GetTotalsData();
            MyStuDataSet dsGrade = GetGradeLegend();
            ReportDataSource datasource = new ReportDataSource("CumulativeDataSet", dsAssessmentcum.Tables[22]);
            ReportDataSource datasource2 = new ReportDataSource("SubjectsDataSet", dsAssessment1.Tables[6]);
            ReportDataSource datasource3 = new ReportDataSource("AsDataset", dsAssessment2.Tables[7]);
            ReportDataSource datasource4 = new ReportDataSource("AttendanceDataSet", dsAttendance.Tables[8]);
            ReportDataSource datasource7 = new ReportDataSource("CumTotalsDataSet", dsCumTotals.Tables[23]);
            ReportDataSource datasource5 = new ReportDataSource("TotalsDataSet", dsTotals.Tables[9]);
            ReportDataSource datasource6 = new ReportDataSource("GradeDataSet", dsGrade.Tables[10]);
            ...
            RptViewerRptCard.LocalReport.DataSources.Add(datasource7);
        ...
        {
            RptViewerRptCard.ProcessingMode = ProcessingMode.Local;
            RptViewerRptCard.LocalReport.ReportPath = Server.MapPath("~/Report/ReportCards_Nursery.rdlc");
            MyStuDataSet dsAssessmentcum = GetData();
            MyStuDataSet dsAssessment1 = GetAssessment1Data();
            MyStuDataSet dsAssessment2 = GetAssessement2Data();
            MyStuDataSet dsAttendance = GetAttendanceData();
            ...
            ReportDataSource datasource5 = new ReportDataSource("TotalsDataSet", dsTotals.Tables[9]);
            ReportDataSource datasource6 = new ReportDataSource("GradeDataSet", dsGrade.Tables[10]);
            ReportDataSource datasource7 = new ReportDataSource("RatingsDataSet", dsNurseryRatings.Tables[18]);
            RptViewerRptCard.LocalReport.DataSources.Clear();
            RptViewerRptCard.LocalReport.DataSources.Add(datasource);
            RptViewerRptCard.LocalReport.DataSources.Add(datasource2);
            RptViewerRptCard.LocalReport.DataSources.Add(datasource3);
