...
{
    public partial class non_returning_students : System.Web.UI.Page
    {
        int lvl = 0;
        ...
        private void NonReturningStudents()
        {
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["conStr"].ConnectionString);
            try
            {
                SqlCommand cmd = new SqlCommand("sp_ms_students_registered_previous_term_not_registered_current_term", con);
                ...
                using (SqlDataAdapter sda = new SqlDataAdapter())
                {
                    ...
                    using (DataTable dt = new DataTable())
                    {
                        ...
                        if (dt.Rows.Count == 0)
                        {
                            ...
                            GridViewNonReturningStudents.Visible = false;
                        }
                        lblZeroRecords.Visible = false;
                        GridViewNonReturningStudents.Visible = true;
                        GridViewNonReturningStudents.DataSource = dt;
                        GridViewNonReturningStudents.DataBind();
                    }
                }
                cmd.ExecuteNonQuery();
            }
