...
{
    public partial class report_card_cumulative : System.Web.UI.Page
    {
        ...
        private MyStuDataSet GetData()
        {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["constr"].ConnectionString))
                try
                {
                    using (SqlCommand cmd = new SqlCommand("sp_ms_person_report_card_breakdown_cum_v7", con))
                    {
                        ...
                        using (SqlDataAdapter sda = new SqlDataAdapter())
                        {
                            ...
                            dt.sp_ms_person_report_card_breakdown_cum_v7.Clear();
                            dt.EnforceConstraints = false;
                            sda.Fill(dt.Tables["sp_ms_person_report_card_breakdown_cum_v7"]);
                        }
                    }
        ...
        private MyStuDataSet GetTotalsData()
        {
            ...
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["constr"].ConnectionString))
                try
                {
                    using (SqlCommand cmd = new SqlCommand("sp_ms_person_report_card_summary_cum_v7", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add("@reg_id", SqlDbType.Int).Value = RegID;
                        // cmd.Parameters.Add("@person_id", SqlDbType.Int).Value = PersonId;
                        using (SqlDataAdapter sda = new SqlDataAdapter())
                        {
                            ...
                            cmd.Connection = con;
                            sda.SelectCommand = cmd;
                            dt.sp_ms_person_report_card_summary_cum_v7.Clear();
                            dt.EnforceConstraints = false;
                            sda.Fill(dt.Tables["sp_ms_person_report_card_summary_cum_v7"]);
                        }
                    }
        ...
        private void Info()
        {
            ...
            ReportDataSource datasource = new ReportDataSource("SubjectsDataSet", dsAssessment1.Tables[24]);
            ...
            ReportDataSource datasource4 = new ReportDataSource("TotalsDataSet", dsTotals.Tables[25]);
